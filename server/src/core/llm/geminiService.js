...
export async function generateSARNarrative(customerData, transactions, ruleEngineOutput) {
  ...
  try {
    ...
    const llmLog = {
      ...
      structured_input: { customer: customerData, transaction_count: transactions.length },
...
function buildCustomerInfo(customer) {
...
function buildTransactionSummary(transactions, ruleOutput) {
...
WHO: ${customer.full_name || customer.name} (${customer.customer_id || customer.id}), ${customer.occupation || 'occupation unknown'}
